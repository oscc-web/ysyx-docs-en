...
First navigate to the [release page of the Logisim-evolution project][logisim release].
Then, locate the latest release version, V4.1.0, and under the `Assets` section, select the installation package compatible with your operating system. Click the corresponding file to initiate the download.
For users running **x86-based Windows systems**, please select `logisim-evolution-4.1.0-amd64.msi`;
For **Mac OS users**, please check whether your Mac has an Apple Silicon (M-series) chip. If so, select `logisim-evolution-4.1.0-aarch64.dmg`. If your Mac has an Intel processor, select `logisim-evolution-4.1.0-x86_64.dmg`.

If your computer does not have the Java runtime environment installed, please install it yourself and configure it properly.

Then run `logisim-evolution-4.1.0-amd64.msi` (Windows) or `logisim-evolution-4.1.0.dmg` (macOS) to install Logisim.
