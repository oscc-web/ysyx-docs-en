...
In our understanding, "high performance" essentially means "runs fast". Therefore, a direct metric for measuring performance is the program's execution time.
So, evaluating a system's performance means evaluating the execution time of programs on that system.
...
SPEC CPU 2017 introduced new programs representing new application scenarios, such as biomedical imaging, 3D rendering and animation, and AI Go programs using Monte Carlo Tree Search (very likely influenced by AlphaGo in 2016).
...
If the processor's application scenario is clear, such as running Family Computer games,
then we can directly use the Family Computer game as the benchmark,
equating the game experience with the standard of "running well".
Unlike microbench, a Family Computer game is a program that never finishes running,
...
The second optimization direction is to reduce the `average cycles per instruction (CPI)`. Another way of saying this is to increase IPC (Instructions Per Cycle), i.e., to increase the average number of instructions executed per cycle.
...
> while `average cycles per instruction` depends only on the processor's capabilities.
...
So how do we quantitatively evaluate the processor's instruction supply, computation efficiency, and data supply?
...
These events are called performance events, and through them,
...
Assuming that the proportion of time that a certain part of the system is actually used is `p`, and the speedup of that part after optimization is `s`,
then the speedup of the entire system is `f(s) = 1 / (1 - p + p / s)`, which is the formula for Amdahl's Law.
...
This counterintuitive conclusion tells us that we cannot simply consider the speedup of a particular component,
...
and the performance improvements achieved through optimizations guided by performance counters will be more realistic.
...
> which significantly optimizes the synthesized netlist generated by yosys, making its timing evaluation results closer to those of commercial tools.
...
ensuring that the number of cycles the NPC waits is close to the number it would wait when running at high frequencies.
...
Since the decimal part is inconvenient to handle, and simply truncating it would introduce significant error,
...
> As a fully customized unit, the maximum operating frequency of SRAM is typically recorded in the corresponding manual,
...
> to guide the synthesizer to synthesize a result with as high a frequency as possible.
...
> We hope to use this method to force you to recognize and understand the performance changes of the NPC,
> developing the fundamental skills required for processor architecture design,
...
> After calibrating the memory access latency based on the clock frequency, you will find that the IPC drops significantly.
...
> Addressing the aforementioned memory frequency inversion through calibration of memory access latency
...
>
> Attempt to modify the relevant simulation workflow to support NPC simulation in both `riscv32e-npc` and `riscv32e-ysyxsoc`.
...
fetching a single instruction requires waiting for dozens or even hundreds of cycles, making it impossible for the pipeline to flow.
...
which possess distinct physical characteristics and thus exhibit differing metrics.
...
but if you were to purchase a 4TB memory module, it would cost `329 * (4TB / 16GB) = 84224` CNY!
...
This is the concept of a cache.
...
and we can temporarily disregard how to handle CPU writes to data blocks.
...
as fetching a new instruction would require multiple memory accesses.
...
The simplest organization method is to read a new block into a fixed cache block, which is called direct-mapped.
...
so that subsequent accesses to the new block can hit in the cache.
...
For a direct-mapped cache with a block size of `b` bytes and `k` cache blocks, we have `tag = addr / b` and `index = (addr / b) % k`.
...
The valid bit and tag are collectively referred to as the cache block's metadata, which means data used to manage data, with the data being managed here being the cache block.
...
> Typically, the LSU is paired with a cache called the data cache (dcache).
...
With DiffTest, you should be able to easily ensure that the given program still runs correctly after integrating the icache.
But how can we ensure that the icache runs correctly for any program?
...
For example, [Z3](https://github.com/Z3Prover/z3) is a solver for [Satisfiability Modulo Theories (SMT)](https://en.wikipedia.org/wiki/Satisfiability_modulo_theories) problems,
...
If a condition can be verified at all time intervals, the method is called unbounded model checking.
However, due to the high computational cost of unbounded model checking, bounded model checking (BMC) is more commonly used in practice.
...
>
> The reason why coverage targets are widely adopted by companies is, on one hand, because coverage is an easy-to-quantify and statistically measurable metric.
...
> ```
> Here, the event can be executing a specific line of code (line coverage), a signal flipping (toggle coverage),
...
Chisel's testing framework [chiseltest](https://github.com/ucb-bar/chiseltest) has integrated formal verification functionality,
...
If a counterexample is found, the waveform generating that counterexample is produced to assist in debugging, which is very convenient.
...
> please refer to the `object test` section in the [new version of `build.sc`](https://github.com/OSCPU/chisel-playground/blob/master/build.sc) to modify your `build.sc`.
...
When calling the formal verification functionality provided by chiseltest, the above code also needs to pass in a `BoundedCheck(1)` parameter as the bound of BMC,
...
Additionally, the above code passes a `BtormcEngineAnnotation` parameter, indicating the invocation of the model checker [BtorMC](https://cs.stanford.edu/~niemetz/publications/2018/NiemetzPreinerWolfBiere-CAV18.pdf).
BtorMC is based on an SMT solver [Boolector](https://github.com/Boolector/boolector) that is different from Z3. Its basic principle is similar to Z3,
...
Specifically, you need to download the corresponding tool from [this link](https://github.com/YosysHQ/oss-cad-suite-build/releases).
...
where [SymbiYosys](https://symbiyosys.readthedocs.io/en/latest/) is the front-end tool for this process.
...
The `depth` option in the above configuration is the bound of BMC, used to specify the number of cycles to be proven.
For example, `depth 4` means that BMC will attempt to prove that the tested module does not violate `assert()` under any input signal within 4 cycles after reset.
For combinational logic circuits, we only need BMC to solve within 1 cycle.

Before performing formal verification, you need to download the corresponding tools from [this link](https://github.com/YosysHQ/oss-cad-suite-build/releases).
...
After correcting the error in the `Sub` module, rerunning the above command will output success information,
...
Here we use Chisel as pseudocode, but if you are developing in Verilog, you can still draw on the relevant ideas to write the verification top-level module.
...
> Regarding the bound of BMC (`BoundedCheck()` or `depth`), you can select an appropriate parameter
> to ensure that the cache can process 3-4 requests within the cycles proven by the formal verification tool,
> thereby testing whether the cache can correctly handle arbitrary consecutive requests.

Formal verification seems to have only advantages, but it actually has a fatal flaw: the state space explosion problem.
As the design scale grows and the bound increases, the space that the solver needs to traverse also grows.
...
Computer scientist [Mark Hill](https://pages.cs.wisc.edu/~markhill/) proposed the 3C model in his 1987 [PhD thesis](https://www2.eecs.berkeley.edu/Pubs/TechRpts/1987/CSD-87-381.pdf),
...
However, the aforementioned cache workflow does not support this functionality, so a new mechanism must be added to achieve it. This mechanism is called prefetching.
However, theoretically, compulsory misses only refer to the misses that occur during the first access to a data block.
In scenarios with a high number of memory accesses, the proportion of compulsory misses is not high.
Therefore, we will not delve into how to reduce compulsory misses here. Interested readers can search for and read relevant materials on prefetching.
...
As mentioned earlier, the direct-mapped organization in the cache means that each data block can only be read into a cache block with a fixed index.
...
Set-associative is a compromise between direct-mapped and fully-associative. The idea is to group all cache blocks,
...
That is, each data block can be stored in any cache block within the group numbered `tag % number of groups`.
...
In fact, fully-associative and direct-mapped can both be regarded as special cases of set-associative: when `w=1`, it is direct-mapped; when `w=total number of cache blocks`, it is fully-associative.
...
However, to read more adjacent data, the miss penalty of the cache also increases;
...
To measure TMT with low overhead, we consider the angle of "miss count * miss penalty,"
and explore how to measure miss count and miss penalty with low overhead.
...
As for the miss penalty, since cachesim does not include memory access details from ysyxSoC, we cannot accurately obtain this value in principle.
However, as discussed earlier, only the block size parameter affects the miss penalty,
so we can calculate an average miss penalty in ysyxSoC and use it as a constant to estimate TMT.
...
> Specifically, we can use cachesim as the REF for performance testing,
> and the performance counter results obtained by running a program in the NPC
...
> which cannot be detected through functional DiffTest with NEMU or formal verification.
> For example, even if the icache consistently misses, the program may still run correctly on the NPC.
> Of course, it is also possible that cachesim, which is used as the REF, has a bug.
> However, regardless of the cause, having a REF for comparison is always beneficial.
>
> However, to obtain a consistent itrace, you may need to make some modifications to NEMU
> to enable it to run the image file of `riscv32e-ysyxsoc`.
...
> However, we have not yet optimized the miss penalty to evaluate a more reasonable TMT;
...
If a cache miss occurs, data must be accessed from the next storage layer, so the miss penalty is the access time of the next storage layer.
There are many techniques to reduce the miss penalty, and we will first discuss one of them: burst access over the bus.
...
> Modify the implementation of the icache to support larger block sizes.
> During implementation, it is recommended to make the block size parameter configurable for subsequent evaluation work.
...
In the AXI bus protocol, the `arburst` signal on the `AR` channel is used to indicate whether the transfer is a burst transfer.
...
When the SDRAM chips return read data, if there are still READ commands with consecutive addresses,
...
finally, the SDRAM controller can respond on the `R` channel while sending the next READ command,
...
In summary, the overhead required for burst transfer is `a+b+4c+d`, which saves `3(a+b+d)` compared to the previous scheme.
Extending this to a general case, if the cache block size is `n` times the bus data width, burst transfer can save `n(a+b+d)` of overhead.
...
As can be seen, to achieve the benefits of burst transfer, the cache must first use larger blocks.
...
To evaluate TMT in cachesim, we also need to obtain the miss penalty when using burst transfer.
To do this, we first need to implement burst transfer in the ysyxSoC environment.
...
> The SoC team has provided a 32-bit tape-out SoC. We also updated `ysyxSoC` to 32-bit on 2024/07/26 13:00:00,
> to help everyone test locally before connecting to the tape-out SoC.
...
>
> To simplify the implementation, you can currently assume that the number of beats in a burst transfer will not exceed 8.
> After implementation, try different values of `r` and observe in the waveform whether the above equations hold true.

> #### todo::Evaluate the performance of the burst transfer method
> After calibrating the memory access delay of the burst transfer method, run the microbench train-scale test and compare the results with the previous records.

#### Quickly Evaluate the Miss Penalty

Based on the previous discussion, the current miss penalty of the icache is only related to the block size and the bus transfer mode, and is independent of other cache parameters.
Therefore, we can pre-evaluate the miss penalty for various combinations of block sizes and bus transfer modes,
and directly substitute this miss penalty into the formula to calculate the TMT, thereby estimating the expected benefits of different cache parameter combinations.
However, based on the above analysis, burst transfer will definitely perform better than independent transfer.
Therefore, in practice, we only need to pre-evaluate the miss penalty of various block sizes when using burst transfer.
...
Therefore, we also recommend using the statistical method to pre-evaluate the miss penalty.

> #### todo::Quickly assess the miss penalty
> Implement a quick assessment of the miss penalty based on the above content.
> You can already calculate the number of misses using cachesim,
> and you will use these miss penalties to estimate the expected benefits of different cache parameter combinations.
### Program Memory Layout

The memory layout of a program can also significantly affect cache performance.
Take the icache as an example. When the icache block size exceeds 4 bytes,
some hot loops in the program may not be aligned to the cache block boundaries,
...
However, if we fill in some blank bytes before the program code, we can change the position of the hot loop,
so that the instructions in the hot loop occupy fewer cache blocks.
For example, in the above example, we only need to fill in 4 bytes of blank content before the program code,
to change the position of the hot loop to `[0x20, 0x38)`.
At this point, the hot loop instructions only occupy 2 cache blocks, and the 1 cache block saved can be used to store other instructions,
...
> Specifically, you can achieve this by modifying the code or modifying the linker script.
...
Specifically, [SPEC CPU defines two metrics, `base` and `peak`](https://www.spec.org/cpu2006/Docs/runrules.html#rule_1.5),
...
> Your NPC needs to be synthesized under the nangate45 process provided by default in the `yosys-sta` project,
> with a total area not exceeding 25000 (the default unit of the area data reported by the synthesis tool), which also serves as the area constraint for the B-stage tape-out.
> Considering that subsequent tasks will require the implementation of pipelining, we recommend that the total synthesis area of the NPC does not exceed 23000 at this stage.
>
> This area is not particularly large. On one hand,
> adopting this area limit helps highlight the contribution of other parameters in the design space exploration;
> otherwise, you could simply keep increasing the cache capacity to achieve good performance,
...
> and a smaller area helps the project team save tape-out costs.
>
...
> however, if the current total synthesis area significantly exceeds the above constraints, you may need to make significant adjustments to your design,
...
> Assuming a foundry offers nangate45 process tape-out services with each block sized `2mm X 3mm` at a price of 500,000 RMB,
> the price per is `500000 / (2 * 3 * 1000000) = 0.0834` RMB.
...
> Using the above estimation method, a design with a synthesis area of 25000
> would require a final cost of `25000 / 0.7 * 0.0834 = 2978` RMB.
>
...
> You can view the goal of the B stage as designing a low-cost embedded processor (rv32e is a basic instruction set for embedded scenarios):
...
> Under these conditions, you need to estimate the cost-effectiveness of adding a functional feature:
> Assuming that a certain feature can improve performance by 10%, is it worth paying an additional 500 RMB for it?

> #### todo::Explore the design space of the icache
> Based on the above introduction, explore the design space of the icache and determine a design plan that achieves good performance under the given constraints.
...
> If your evaluated area greatly exceeds the above requirements, you will most likely need to optimize your design.
...
> yzh's reference design achieves an area of 22730 after adding the icache, while maintaining a frequency of 1081MHz,
> and the `Total time` displayed when running microbench is 4.49s.
> We set the above area constraints, on one hand, to train everyone in how to optimize their own designs:
> if you are a beginner, you need an opportunity to get started with this kind of work,
> and through continuous trial and error, you will develop an understanding of the relationship between each line of RTL code and its area overhead.
>
> On the other hand, it is also to let everyone once again experience the goal of architectural design:
> performance and area are mutually constrained. If you find your design difficult to optimize,
> the simplest method is to reduce the cache capacity, but you will have to pay a performance penalty;
...
> More fundamentally, the former belongs to the training of architectural design capabilities, while the latter does not.

> #### todo::Evaluate the cost-effectiveness of the dcache
> You have estimated the performance gain of the dcache under ideal conditions in the previous section.
> Here, we continue to estimate the cost-effectiveness of this dcache.
> Assuming that the area of this dcache is the same as that of the icache, what is its cost in RMB?
>
...
> If we consider the real performance gain and actual area of the dcache, the cost-effectiveness of designing a dcache will be even lower.
>
...
> Although the above icache design space exploration task is significantly simplified compared to the design space exploration in real processors,
> for most students this is still the first time they have encountered real processor architecture design.
> More importantly, this is likely the first time most students have experienced the entire design process of a module,
> from requirement analysis, structural design, logic design, to functional verification, performance verification, performance optimization,
> and finally to area evaluation and timing analysis at the circuit level.
...
> With simulators, we do not need to simulate circuit-level behavior (run cachesim instead of verilator),
> we only need to simulate necessary modules (do not simulate the cache data, only simulate the cache metadata),
> and we do not need processor-driven execution (do not run the full program, only replay the corresponding itrace).
> It is these differences that make simulators an order of magnitude more efficient than RTL simulation,
...
> but running the same program on the full-system simulator [gem5](https://www.gem5.org/) takes only 2 hours.
...
> The ISCA conference has held multiple competitions based on the [ChampSim](https://github.com/ChampSim/ChampSim) simulator,
> including the [cache replacement algorithm competition](https://www.sigarch.org/call-contributions/the-2nd-cache-replacement-championship/)
> and the [data prefetch algorithm competition](https://www.sigarch.org/call-contributions/third-data-prefetching-championship/).
...
subsequent reads from the corresponding address should obtain the new data; otherwise, program execution will fail.
However, due to the cache mechanism, multiple copies of the data may exist in memory.
How to ensure that the new data can be read from each copy is known as the cache coherence problem.
...
After adding the icache to the NPC, we can reproduce a consistency issue using the following `smc.c`:
...
According to the program's semantics, the program should output a character `A` and then return from `main()` via the rewritten `ret` instruction.
This kind of code that modifies itself during execution is called "self-modifying code."
...
> enabling programs to implement more functionality within limited memory.
> For example, the [FC Family Computer console in the 1980s had only a 64KB address space](https://www.nesdev.org/wiki/CPU_memory_map), of which the ROM on the cartridge occupied 32KB.
...
> To develop wonderful games with such limited resources,
> developers employed numerous black technologies, and self-modifying code was one of them.
>
> With the development of memory technology, memory capacity is no longer as tight as it was in the past,
...
> Compile the above program for the AM and run it on the NPC. What problems did you find?
> Try to analyze the problems and verify your ideas using waveforms.
>
...
If there are, update them or invalidate them to ensure that subsequent operations, regardless of where they access from,
can directly read the new data (using the update method) or read the new data from the next storage layer due to a miss (using the invalidation method).
...
The RISC-V instruction set adopts this solution. RISC-V includes a `fence.i` instruction,
whose semantics ensure that all subsequent instruction fetches can see the data modified by store instructions that occurred before it.
Here, the `fence.i` instruction acts as a barrier, preventing subsequent instruction fetches from crossing the barrier to read the old data before it was modified by store instructions.
...
For more information about `fence.i`, please RTFM.
...
since all copies are already consistent when the store instruction is executed, `fence.i` can be implemented as `nop`.
...
ensuring that subsequent accesses to the icache definitely miss, thereby accessing the new data in memory.
...
if the instruction set specification requires the processor to handle copy consistency at the software level, the hardware design is simpler, but the cost is increased programmer burden.
Therefore, the essence of this issue is a trade-off between hardware design complexity and program development burden.

> #### todo::Implement the `fence.i` instruction
> Based on your understanding of the `fence.i` instruction, choose a reasonable scheme to implement the `fence.i` instruction in the NPC.
> After implementation, add the `fence.i` instruction to the appropriate location in the aforementioned `smc.c` and rerun it on the NPC.
> If your implementation is correct, you will see the program output a character `A` and then end successfully.
>
...
You already know that the bootloader's job is to move the program's code and data to the target memory location, and then jump to the program's code to continue execution.
Moving requires store instructions, and even when moving program code, store instructions are still needed.
This means that without adding a `fence.i` instruction, when jumping from the bootloader to execute the program's code, the RISC-V processor does not guarantee that the correct instructions will be fetched.

For the loader in an operating system, this problem is even more obvious.
The operating system may first load program A to a certain memory location, and then jump to program A to execute it;
after program A finishes executing, the operating system decides to load program B to the same memory location.
Without adding a `fence.i` instruction, during the execution of program B, instructions from program A may be incorrectly fetched.
...
>
> After the modification, try using `dummy` as the target program. Then try enlarging your icache until it can hold all the instructions in the process.
> You should see that as the icache capacity increases, the `dummy` program fails to run.
>
> Although the `nop` instruction sequence here is not a real program, considering that when an operating system is run in the future,
> analyzing this problem from the failure symptoms would become difficult, we choose to resolve this problem here.

> #### todo::Add the `fence.i` instruction in the bootloader
> After reproducing the problem, try adding the `fence.i` instruction at an appropriate location in the bootloader, compile, and rerun.
> If your implementation is correct, you will see the `dummy` program run successfully with a large-capacity icache.
>
...
In fact, in real computers, the cache coherence problem has even more manifestations.
As processors become more complex later, we will also discuss more cache coherence problems.
