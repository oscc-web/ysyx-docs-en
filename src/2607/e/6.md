# E6 A Simple Runtime Environment
...
Let's first try a classic game. Before that, you need to install `python`:
...
We will first compile and run a simple C program. This simple C program is `am-kernels/tests/cpu-tests/tests/dummy.c`. After reading its code, you will find that it does nothing and returns immediately.
...
For a real UART, before outputting a character, the program also needs to check whether the device is ready. If a character is output when the UART is not ready, that character will be lost. Whether the UART is ready is queried by reading the UART status register. Similarly, we can add the corresponding functionality to the UART behavior model:
