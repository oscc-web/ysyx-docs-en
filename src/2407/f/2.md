...
Then, locate the latest release version, V3.8.0, and under the `Assets` section, select the installation package compatible with your operating system. Click the corresponding file to initiate the download.
...
Then run `logisim-evolution-3.8.0-x86.msi` (Windows) or `logisim-evolution-3.8.0.dmg` (macOS) to install Logisim.
