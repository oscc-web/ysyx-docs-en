...
import navbarObjZh from "../../config/navbar-zh-tmp"
import navbarObjEn from "../../config/navbar-en-tmp"
let navbarObjCurr = navbarObjZh
let navbarObjLast = {}
let navbarObjCurrKey  = "2306"
let navbarObjLastKey  = ""
let navbarObjCurrItem = navbarObjCurr[navbarObjCurrKey]
...
const useNavbarConfig = (): ComputedRef<ResolvedNavbarItem[]> => {
  ...
  router.beforeEach((to) => {
    ...
    if (path.indexOf("/en/") != -1) {
      navbarObjCurr = navbarObjEn
    }
    else {
      navbarObjCurr = navbarObjZh
    }

    if (path.indexOf("ics-pa") != -1) {
    }
    else if (path.indexOf("2306") != -1 || path === "/" || path === "/en/") {
      navbarObjCurrKey = "2306";
    }
    else if (path.indexOf("2205") != -1) {
      navbarObjCurrKey = "2205";
    }
    else {
    }

    if ((navbarObjCurrKey != navbarObjLastKey) ||
        (navbarObjCurr != navbarObjLast)) {
      navbarObjCurrItem = navbarObjCurr[navbarObjCurrKey]
    }

    navbarObjLastKey = navbarObjCurrKey
    navbarObjLast = navbarObjCurr
  });
  return computed(() => (navbarObjCurrItem || []).map(resolveNavbarItem))
